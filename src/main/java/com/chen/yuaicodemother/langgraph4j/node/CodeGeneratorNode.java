...
import com.chen.yuaicodemother.langgraph4j.model.QualityResult;
...
/**
 * 代码生成节点
 * 通过构造函数注入 AiCodeGeneratorFacade，避免 SpringContextUtil 反模式
 */
@Slf4j
public class CodeGeneratorNode {
     ...
     * 创建代码生成节点
     *
     * @param codeGeneratorFacade AI 代码生成服务（通过构造函数注入）
     * @return 异步节点动作
     */
    public static AsyncNodeAction<MessagesState<String>> create(AiCodeGeneratorFacade codeGeneratorFacade) {
        return node_async(state -> {
            ...
            // 根据类型设置生成目录
            String generatedCodeDir = String.format("%s/%s_%s", AppConstant.getCodeOutputRootDir(), generationType.getValue(), appId);
